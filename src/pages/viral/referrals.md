...
!!! warning "Only use v1 custom events"
	Our referral system only supports our v1 custom event SDK method which is `userCompletedAction()`. The v2 events, `BranchEvent` and `logEvent()`, are not supported at this time.
